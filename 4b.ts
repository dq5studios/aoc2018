...
// [1518-11-05 00:55] wakes up`;
    ...
    let asleep_at: number;
    let total_sleep: number[][] = [];
    list.forEach((value) => {
        let matches = value.match(/\[([^\]]*)\] (.*)/);
        let timestamp = new Date(matches[1]);
        if (matches[2].search(/Guard #[0-9]+/) > -1) {
            // Guard change
            guard = Number(matches[2].match(/Guard #([0-9]+)/)[1]);
            if (typeof total_sleep[guard] === "undefined") {
                total_sleep[guard] = new Array(60).fill(0);
            }
        }
        if (matches[2] == "falls asleep") {
            asleep = true;
            asleep_at = timestamp.getMinutes();
        }
        if (asleep && matches[2] == "wakes up") {
            asleep = false;
            for (let i = asleep_at; i < timestamp.getMinutes(); i++) {
                total_sleep[guard][i]++;
            }
        }
    });
